...
public class MoveAfterEditOperation : EditOperation
{
    ...
    public override void Parse(string dslText)
    {
        ...
        if (!match.Success)
        {
            throw new ParseException("Invalid move-after operation format. Expected: \"text\"@position->@targetposition or @position->@targetposition", dslText);
        }
